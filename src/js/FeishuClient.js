...
// 初始化飞书SDK实例（从环境变量读取配置，避免硬编码）
export const feishuClient = new FeishuClient({
    appId: import.meta.env.VITE_FEISHU_APP_ID,          // 前端环境变量（仅存储appId）
    redirectUri: import.meta.env.VITE_FEISHU_REDIRECT_URI || 'http://localhost:5173', // 回调地址（优先环境变量）
});